...
namespace TradingLib.Common
{
    ...
    /// </summary>
    public class BarListImpl : TradingLib.API.BarList
    {
        /// <summary>
        ...
        public static BarListImpl InsertBar(BarList bl, Bar b, int position)
        {
            ...
            for (int j = 0; j < bl.CustomIntervals.Length; j++)//获得所有的intervals 
            {
                if (bl.CustomIntervals[j] != b.Interval) //如果bar的interval与barlist所管理的intervals 不一致则尝试下一个 interval
                    continue;
                
                int count = bl.IntervalCount(b.Interval);//获得某个interval的bar的数目
                if (count > 0) //如果当前 interval bar的数目>0
                {
                    for (int i = 0; i < count; i++)
                    {
                        if (i == position)//如果序号和postition一致则插入
                        {
                            addbar(copy, b, j);
                        }
        ...
        //新增日期
        public double[] oaDateTime() { return _intdata[_intdataidx[_defaultint]].oaDateTime().ToArray(); }
        ...
        public long[] Vol(BarInterval interval) { return _intdata[_intdataidx[(int)interval]].vol().ToArray(); }
        ...
        #region 获得最新的bar和interval对应的bar的数目
        /// <summary>
        ...
        #region 查询是否有某个interval的最少多少个bar
        /// <summary>
        ...
        public void FillBars(IList<Bar> barlist)
        {
            foreach (Bar b in barlist)
            {
                addbar(this, b, _intdataidx[_defaultint]);
            }
        }
...
histperiod=daily&startdate=" + startdate + "&enddate=" + enddate + "&output=csv&q=" + symbol;
        ...
        **/
        ...
        /**
        static string GetChartDb() { return GetChartDb(Util.ProgramData(CHARTDBDEFAULTNAME)); }
        ...
        public static BarList GetChart(string sym, bool trycache, int delayms, DebugDelegate deb) { return GetChart(sym,Util.ToTLDate(), MaxCacheDays_Chart, trycache, delayms, deb); }
        public static BarList GetChart(string sym, DebugDelegate deb) { return GetChart(sym, Util.ToTLDate(), MaxCacheDays_Chart, TryCache_Chart, DelayMs_Chart, deb); }
        public static BarList GetChart(string sym) { return GetChart(sym,Util.ToTLDate(), MaxCacheDays_Chart, TryCache_Chart, DelayMs_Chart, null); }
        ...
        public static BarList GetChart(string sym, int date, int maxcacheagedays, bool trycache, int delayms, DebugDelegate deb)
        {
            ...
            if (trycache && System.IO.File.Exists(chart))
            {
                try
                {
                    var bl = BarListImpl.FromTIK(chart);
            ...
            // grab it
            var newbl = BarListImpl.DayFromGoogle(sym);
