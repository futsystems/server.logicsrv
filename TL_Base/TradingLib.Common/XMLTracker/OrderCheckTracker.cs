...
{
    public class OrderCheckTracker
    {
        const string FN = @"config\OrderCheck.xml";
        ...
        public static void delRuleFromAccount(string acc, IOrderCheck rc)
        {
        ...
        public static void addRuleIntoAccount(string acc, IOrderCheck rc)
        {
            if (!HaveAccount(acc))
                addAccount(acc);
            ...
            {
                //return;
                delRuleFromAccount(acc, rc);//如果有该委托检查,则先删除该委托检查然后将新的委托检查添加到账户记录
            }
            XmlNode bnode = null;
            XmlDocument xmlDoc = getXMLDoc();
            XmlNode xn = xmlDoc.SelectSingleNode("RuleSet");
        ...
        public static bool AccountHaveRule(string acc, IOrderCheck rc)
        {
            XmlDocument xmlDoc = getXMLDoc();
            XmlNode xn = xmlDoc.SelectSingleNode("RuleSet");
            XmlNodeList exlist = xn.ChildNodes;
