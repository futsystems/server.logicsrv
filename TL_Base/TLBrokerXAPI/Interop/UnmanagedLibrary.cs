...
namespace TradingLib.BrokerXAPI.Interop
{
    ...
    internal sealed class UnmanagedLibrary : IDisposable
    {
        ...
        public UnmanagedLibrary(string path,string fileName)
        {
            if (fileName == null)
            {
            ...
            _handle = LoadFromPath(path, fileName);
            ...
            if (_handle == null || _handle.IsInvalid)
            {
                throw new FileNotFoundException(
                    "Unable to find " + fileName + " on  path " + path + ".",
                    fileName,
        ...
        private SafeLibraryHandle LoadFromPath(string dir, string filename)
        {
            ...
            Util.Debug("libPath:" + libPath,this.GetType().Name);
