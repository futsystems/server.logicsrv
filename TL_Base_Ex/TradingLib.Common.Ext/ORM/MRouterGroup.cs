...
namespace TradingLib.ORM
{
    public class MRouterGroup:MBase
    {
        ...
        public static void UpdateRouterItem(RouterItemImpl item)
        {
            using (DBMySql db = new DBMySql())
            {
                string query = string.Format("UPDATE router_items SET  marginlimit='{0}',priority='{1}',rule='{2}' ,active='{3}'  WHERE id='{4}'",item.MarginLimit, item.priority, item.rule, item.Active ? 1 : 0, item.ID);
                db.Connection.Execute(query);
            }
        }
        ...
        public static void InsertRouterItem(RouterItemImpl item)
        {
            using (DBMySql db = new DBMySql())
            {
                string query = String.Format("Insert into router_items (`routegroup_id`,`connector_id`,`marginlimit`,`priority`,`rule`,`active`) VALUES ('{0}','{1}','{2}','{3}','{4}','{5}')", item.routegroup_id, item.Connector_ID, item.MarginLimit, item.priority,item.rule, item.Active ? 1 : 0);
                db.Connection.Execute(query);
