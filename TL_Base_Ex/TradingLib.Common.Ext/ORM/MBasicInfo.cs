...
namespace TradingLib.ORM
{
    ...
    public class MBasicInfo : MBase
    {
        ...
        public static bool UpdateSymbolForce(SymbolImpl sym)
        {
            using (DBMySql db = new DBMySql())
            {
                //TLCtxHelper.Ctx.debug("orm update mktimefk:" + sec.MarketTimeFK.ToString() + " exchangefk:" + sec.ExchangeFK.ToString() + " underfk:" + sec.UnderLayingFK.ToString());
                string query = string.Format("UPDATE info_symbols SET entrycommission='{0}',exitcommission='{1}',margin='{2}',extramargin='{3}',maintancemargin='{4}' ,expiremonth='{5}',expiredate='{6}',tradeable='{7}',security_fk='{8}' WHERE id='{9}'", sym._entrycommission, sym._exitcommission, sym._margin, sym._extramargin, sym._maintancemargin, 0, sym.ExpireDate, sym.Tradeable ? 1 : 0, sym.security_fk, sym.ID);
                return db.Connection.Execute(query) >= 0;
            }
        }
