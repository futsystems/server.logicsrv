        ...
        public static bool ConfirmAccountCashOperation(JsonWrapperCashOperation op)
        {
            ...
            using (DBMySql db = new DBMySql())
            {
                using (var transaction = db.Connection.BeginTransaction())
                {
                    ...
                    string query2 = String.Format("Insert into log_cashtrans (`datetime`,`amount`,`comment`,`account`,`transref`,`settleday`) values('{0}','{1}','{2}','{3}','{4}','{5}')", Util.ToTLDateTime(), amount.ToString(), op.Source.ToString(), op.Account, op.Ref, TLCtxHelper.ModuleSettleCentre.NextTradingday);
