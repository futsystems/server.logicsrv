...
namespace TradingLib.ORM
{
    public class MManager:MBase
    {
        ...
        public static bool UpdateManager(Manager manager)
        {
            using (DBMySql db = new DBMySql())
            {
                string query = String.Format("UPDATE manager SET login= '{0}' , type = '{1}' , name = '{2}' , mobile = '{3}' , qq = '{4}' , acclimit = '{5}' WHERE id = '{6}'",manager.Login,manager.Type.ToString(),manager.Name,manager.Mobile,manager.QQ,manager.AccLimit,manager.ID);
                return db.Connection.Execute(query) >= 0;
            }
        }
        ...
        public static bool InsertManager(Manager manger)
        {
            ...
            using (DBMySql db = new DBMySql())
            {
                string query = String.Format("Insert into manager (`login`,`type`,`name`,`mobile`,`qq`,`acclimit`,`parent_fk`) values('{0}','{1}','{2}','{3}','{4}','{5}','{6}')", manger.Login, manger.Type.ToString(), manger.Name, manger.Mobile, manger.QQ, manger.AccLimit,manger.parent_fk);//orderpostflag`,`forceclose`,`hedgeflag`,`orderref`,`orderexchid`,`orderseq`
