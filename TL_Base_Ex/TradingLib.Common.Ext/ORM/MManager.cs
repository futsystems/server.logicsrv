...
namespace TradingLib.ORM
{
    public class MManager:MBase
    {
        ...
        public static bool UpdateManager(Manager manager)
        {
            using (DBMySql db = new DBMySql())
            {
                string query = String.Format("UPDATE manager SET  name = '{0}' , mobile = '{1}' , qq = '{2}',acclimit = '{3}',agentlimit = '{4}' WHERE id = '{5}'", manager.Name, manager.Mobile, manager.QQ, manager.AccLimit,manager.AgentLimit, manager.ID);
                return db.Connection.Execute(query) >= 0;
            }
        }
        ...
        public static bool InsertManager(Manager manger)
        {
            using (DBMySql db = new DBMySql())
            {
                string query = String.Format("Insert into manager (`login`,`type`,`name`,`mobile`,`qq`,`acclimit`,`domain_id`,`active`,`agentlimit`) values('{0}','{1}','{2}','{3}','{4}','{5}','{6}','{7}','{8}')", manger.Login, manger.Type.ToString(), manger.Name, manger.Mobile, manger.QQ, manger.AccLimit, manger.domain_id, manger.Active ? 1 : 0,manger.AgentLimit);
