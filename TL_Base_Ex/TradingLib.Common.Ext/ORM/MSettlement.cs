...
namespace TradingLib.ORM
{
    ...
    public class MSettlement:MBase
    {
        ...
        public static void InsertSettlementPrice(MarketData data)
        {
            using (DBMySql db = new DBMySql())
            {
        ...
        /// </summary>
        /// <param name="price"></param>
        public static void UpdateSettlementPrice(MarketData data)
        {
            using (DBMySql db = new DBMySql())
            {
                string query = String.Format("UPDATE log_settlement_price SET settlement = '{0}' WHERE symbol = '{1}' AND settleday = '{2}'", data.Settlement,data.Symbol,data.SettleDay);
                db.Connection.Execute(query);
            }  
        }
