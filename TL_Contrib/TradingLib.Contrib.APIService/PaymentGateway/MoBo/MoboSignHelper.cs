...
namespace TradingLib.Contrib.APIService
{
    ...
    public sealed class MobaopaySignUtil
    {
        ...
        /// <returns></returns>
        public string sign(string sourceData,string key)
        {
            MD5 md5 = MD5.Create();
            byte[] data = Encoding.UTF8.GetBytes(sourceData + key);
        ...
        /// <returns></returns>
        public bool verifyData(string signData, string srcData,string key)
        {
            string newSignData = sign(srcData,key);
            // 忽略字母的大小写
