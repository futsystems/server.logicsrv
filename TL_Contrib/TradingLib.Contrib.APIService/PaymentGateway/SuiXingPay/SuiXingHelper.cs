...
namespace TradingLib.Contrib.APIService
{
    public class SuiXingHelper
    {
        ...
        /// <param name="privateKey"></param>
        /// <returns></returns>
        public static string Sign(byte[] inputBytes, string privateKey)
        {
            using (var rsaProvider = new RSACryptoServiceProvider())
            {
                rsaProvider.FromXmlString(privateKey);
                byte[] signatureBytes = rsaProvider.SignData(inputBytes, "SHA1");
                return Convert.ToBase64String(signatureBytes);  
            }

        }

        public static bool Verify(byte[] inputBytes,byte[] sign, string publicKey)
        {
            using (var rsaProvider = new RSACryptoServiceProvider())
            {
                rsaProvider.FromXmlString(publicKey);
                return rsaProvider.VerifyData(inputBytes, "SHA1", sign);
            }
        }
