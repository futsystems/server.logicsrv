...
namespace TradingLib.Contrib.Payment.ZhiHPay
{
    ...
    {
        static ILog logger = LogManager.GetLogger("ZhiHPayGateWay");

        public ZhiHPayGateWay(GateWayConfig config)
            : base(config)
        {
            this.GateWayType = QSEnumGateWayType.ZhiHPay;
            ...
            this.ZhiHPayKey = data["ZhiHPublickKey"].ToString();
        ...
        public override Drop CreatePaymentDrop(CashOperation operatioin, HttpRequest request)
        {
            ...
            data.return_url = APIGlobal.CustNotifyUrl + "/zhihpay";
            data.notify_url = APIGlobal.SrvNotifyUrl + "/zhihpay";
            ...
            string key = ZhiHPayHelper.RSAPrivateKeyJava2DotNet(this.PrivateKey);
            string signdata = ZhiHPayHelper.RSASign(signSrc, key);
        ...
        public override bool CheckParameters(NHttp.HttpRequest request)
        {
            ...
            **/
            ...
            //将智付公钥转换成C#专用格式
            var key = ZhiHPayHelper.RSAPublicKeyJava2DotNet(this.ZhiHPayKey);
            //验签
            bool result = ZhiHPayHelper.ValidateRsaSign(signStr, key, dinpaysign);
            return result;
        }
