...
namespace TradingLib.Contrib.Payment.JuHe
{
    ...
    class HttpHelper
    {
        ...
        /// <param name="postDict"></param>
        /// <returns></returns>
        public static string PostHttpResponseJson(string url, IWebProxy webProxy, IDictionary<string, string> postDict, string api_key)
        {
            string json = string.Empty;
            Encoding encoding = Encoding.UTF8;
            HttpWebResponse Response = CreatePostHttpResponse(new HttpPostParametersModel()
        ...
        /// <param name="cookies">随同HTTP请求发送的Cookie信息，如果不需要身份验证可以为空</param>
        /// <returns></returns>
        public static HttpWebResponse CreatePostHttpResponse(HttpPostParametersModel postParametersModel, String api_key)
        {
            if (string.IsNullOrEmpty(postParametersModel.Url))
            {
                throw new ArgumentNullException("postParametersModel.Url");
            ...
            {
                request.Credentials = GetCredentialCache(postParametersModel.Url, api_key, "");
                request.Headers.Add("Authorization", GetAuthorization(api_key, ""));
            }
            if (postParametersModel.WebProxy != null)
            {
                request.Proxy = postParametersModel.WebProxy;
