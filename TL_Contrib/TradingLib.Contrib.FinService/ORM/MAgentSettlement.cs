...
namespace TradingLib.Contrib.FinService.ORM
{
    public class MAgentSettlement:MBase
    {
        ...
        {
            return IsAgentSettled(agentfk, TLCtxHelper.ModuleSettleCentre.NextTradingday);
        }
        ...
        public static bool SettleAgent(Manager mgr)
        {
            ...
            JsonWrapperToalReport report = ORM.MServiceChargeReport.GenTotalReport(mgr.mgr_fk, TLCtxHelper.ModuleSettleCentre.NextTradingday);

            settle.LastEquity = mgr.GetAgentBalance().Balance;
            settle.mgr_fk = mgr.mgr_fk;
            settle.Settleday = TLCtxHelper.ModuleSettleCentre.NextTradingday;
