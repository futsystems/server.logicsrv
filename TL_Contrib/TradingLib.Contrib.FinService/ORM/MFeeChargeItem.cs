...
namespace TradingLib.Contrib.FinService.ORM
{
    public class MFeeChargeItem:MBase
    {
        ...
        public static IEnumerable<FeeChargeItem> SelectFeeChargeItemAfterSettle()
        {
            using (DBMySql db = new DBMySql())
            {
                string query = string.Format("SELECT * FROM log_service_feecharge WHERE settleday = '{0}' AND collecttype='{1}'", TLCtxHelper.ModuleSettleCentre.NextTradingday, EnumFeeCollectType.CollectAfterSettle);
