...
namespace TradingLib.Contrib.FinService
{
    ...
    /// 覆写相关函数 实现对应的逻辑
    /// </summary>
    public class SPBonus : ServicePlanBase
    {

        /// <summary>
        /// 单位资金成本 多少元/万元
        /// </summary>
        [ArgumentAttribute("BonusRate", "盈利分红比例", EnumArgumentType.DECIMAL, true, 0.2, 0.2)]
        public ArgumentPair BonusRate { get; set; }
        ...
        {
            decimal profit = this.Account.Profit;
            decimal target = this.Account.Profit > 0 ? this.Account.Profit : 0;
            ...
            AgentCommissionDel func = (Manager agent, Manager parent) =>
            {
                decimal fee = 0;
                //代理的收费 - 代理的父代理的收费
                decimal diff = FinTracker.ArgumentTracker.GetAgentArgument(agent.mgr_fk, this.ServicePlanFK, this.BonusRate.AccountArgument.Name).AsDecimal() - FinTracker.ArgumentTracker.GetAgentArgument(parent.mgr_fk, this.ServicePlanFK, this.BonusRate.AccountArgument.Name).AsDecimal();
                fee = diff * target;
                return fee;
            };
