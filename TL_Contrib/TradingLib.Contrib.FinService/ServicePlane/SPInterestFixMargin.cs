...
namespace TradingLib.Contrib.FinService
{
    ...
    /// 覆写相关函数 实现对应的逻辑
    /// </summary>
    public class SPInterestFixMargin : ServicePlanBase
    {
        ...
        /// 此比例 为安全本金的比例
        /// </summary>
        [ArgumentAttribute("StopEquity", "强平权益", EnumArgumentType.DECIMAL, true, 0, 0)]
        ...
        {
            //检查强平额度和当前权益 如果当前权益低于强平额度 则冻结交易帐户
            ...
            {
                Util.Debug(string.Format("帐户:{0} 当前权益:{1} 配资额度为:{2} 强平权益额度为:{3} 不满足本金要求,冻结交易帐户", this.Account.ID, nowequity, this.FinAmount.AccountArgument.AsDecimal(), stopmargin));
        ...
        public override bool CanTakeOrder(Order o, out string msg)
        {
            ...
            Util.Debug("SPInterestFixMargin Fundavabile:" + avabile.ToString() + " Required:" + required + " account avabile fund:" + this.Account.AvabileFunds.ToString());
            if (required > avabile)
            {
                msg = "资金不足";
                return false;
