...
namespace TradingLib.Quant.Chart
{
    ...
    public class QSCommonDataProvider : CommonDataProvider
    {
        ...
        QSChartData chartdata;
        /// <summary>
        /// 从实现中间数据接口IEasyChartData 对象中加载数据
        /// 在加载数据的过程中每次都需要清空一次htdata然后将对应的数据项复制到htdata
        /// </summary>
        /// <param name="data"></param>
        public void LoadQSChartData(QSChartData data)
        {
            //MessageBox.Show("load data here");

            chartdata = data;
            //if (ds.Length > 4)
            
                this.htData.Clear();
                this.htData.Add("OPEN",chartdata.Open);
                this.htData.Add("HIGH", chartdata.High);
                this.htData.Add("LOW", chartdata.Low);
                this.htData.Add("CLOSE", chartdata.Close);
                this.htData.Add("VOLUME", chartdata.Volume);
                this.htData.Add("DATE", chartdata.Date);
                /*
                if (ds.Length > 6)
                {
        ...
        public override double[] GetData(string DataType)
        {
            ...
            Hashtable cycleData = htData;//this.GetCycleData(this.DataCycle);如果使用getcycleData则会根据设定的datacycle自动调整数据
            //QSChartData cycleData = chartdata;
            if (cycleData == null)
            {
                throw new Exception(string.Concat(new object[] { "QuantShop Quote data", DataType, " ", this.DataCycle, " not found" }));
            }
            double[] dd = (double[])cycleData[DataType.ToUpper()];
            if (dd == null)
            {
                throw new Exception("The name xxxx" + DataType + " does not exist.");
            }
            if ((this.BaseDataProvider != null) && (this.BaseDataProvider != this))
            {
                //dd = this.AdjustByBase((double[])cycleData["DATE"], dd);
                throw new Exception("do not support AdjustByBase");
            }
            if(this.MaxCount !=-1)
            {
                if (dd.Length > this.MaxCount)
                {
                    throw new Exception("MaxBarCount reached");
                }
            }
            
            
            //数据内存复制 是否是防止多线程操作一块数据 造成多线程不安全
        ...
        {
            //MessageBox.Show("getcycledata called");
