...
namespace TradingLib.Core
{
        ...
        where T : ClientInfoBase, new()
    {
        ...
        /// 返回所有客户端
        /// </summary>
        public T[] Clients { get { return clientMap.Values.ToArray(); } }
        ...
        public int NumClientLoggedIn
        {
            ...
            {
                int r = 0;
                //foreach (T c in clientMap.Values)
                //{
                //    if (c.IsLoggedIn)
                //        r++;
                //}
        ...
        public void DropDeadClient(DateTime deadtime)
        {
            ...
            IEnumerable<T> clients = clientMap.Values.Where(e=>e.FrontType != Common.QSEnumFrontType.EVAccess && e.HeartBeat<=deadtime);
            //from client in clientMap.Values
            //where client.HeartBeat <= deadtime
            //select client;
    ...
    /// </summary>
    public class ClientList<T>:IClientList
    {
        /// <summary>
        /// 某个终端注册事件
        /// </summary>
        ...
        private ConcurrentDictionary<string, ClientInfoBase> clientMap = new ConcurrentDictionary<string, ClientInfoBase>();
        public ClientInfoBase[] Clients { get { return clientMap.Values.ToArray(); } }
        ...
        /// <param name="ClientID"></param>
        /// <returns></returns>
        public ClientInfoBase this[string ClientID]
        {
            get
            {
                ClientInfoBase c;
        ...
        /// 交易客户端有不同类别的登入前端FastAccess WebAccess需要可以同时进行连接并交易
        /// </summary>
        /// <param name="account"></param>
        public void DelClientByLoginID(string loginid,QSEnumFrontType fronttype,bool allowmutlfront)
        {
            List<IClientInfo> tobedelete = new List<IClientInfo>();
            foreach (IClientInfo c in clientMap.Values)
            {
                ...
                {
                    if (c.LoginID == loginid && c.FrontType == fronttype)
                    {
                        tobedelete.Add(c);
                    }
