...
namespace TradingLib.Core
{
    public partial class ClearCentre
    {
        ...
        public void CTE_OpenClearCentre(ISession session)
        {
            try
            {
                debug(string.Format("管理员:{0} 请求开启清算中心", session.AuthorizedID), QSEnumDebugLevel.INFO);
        ...
        public void CTE_ClearClearCentre(ISession session)
        {
            try
            {
                debug(string.Format("管理员:{0} 请求关闭清算中心", session.AuthorizedID), QSEnumDebugLevel.INFO);
