...
namespace TradingLib.Core
{
    ...
    /// </summary>
    public partial class BrokerRouter:BaseSrvObject
    {
        ...
        public IBroker SelectBroker(Order o)
        {
            ...
            {
                int rg_fk = account.RG_FK;
                RouterGroup rg = BasicTracker.RouterGroupTracker[rg_fk];
                //没有设定路由组则返回null
                if (rg == null)
                {
                    debug(string.Format("account:{0} have not set router gorup fk:{1}", account.ID, rg_fk), QSEnumDebugLevel.WARNING);
                    return null;
                }
