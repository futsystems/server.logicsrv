...
namespace TradingLib.Core
{
    ...
    public partial class SettleCentre : BaseSrvObject, ISettleCentre,ICore
    {
            ...
            :base(SettleCentre.CoreName)
        {
            ...
            TaskProc taskstore = new TaskProc(this.UUID, "交易系统转储交易记录-" + Util.ToTLTime(storetime).ToString(), storetime.Hour, storetime.Minute, storetime.Second, delegate() { Task_DataStore(); });
            ...
            TaskProc tasksettle = new TaskProc(this.UUID, "交易系统结算-" + Util.ToTLTime(settletime).ToString(), settletime.Hour, settletime.Minute, settletime.Second, delegate() { Task_SettleAccount(); });
            TLCtxHelper.Ctx.InjectTask(tasksettle);

            //注入重置任务 可以在数据指定重置时间
            DateTime resettime = Util.ToDateTime(Util.ToTLDate(DateTime.Now), _resetTime);
            TaskProc taskreset = new TaskProc(this.UUID, "交易系统重置-" + Util.ToTLTime(resettime).ToString(), resettime.Hour, resettime.Minute, resettime.Second, delegate() { Task_ResetTradingday(); });
