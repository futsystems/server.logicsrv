...
namespace TradingLib.Core
{
    ...
    internal class PositionFlatSet
    {
        ...
        /// 需要车的委托列表
        /// </summary>
        public List<long> OrderCancels {get;set;}

        /// <summary>
        /// 是否有需要撤掉的委托
        /// </summary>
        public bool HaveOrderCancels { get; set; }

        /// <summary>
        /// 是否有待强平持仓
        /// </summary>
        public bool HavePendingPostionFlat { get; set; }
        ...
        /// <param name="source"></param>
        /// <param name="closereason"></param>
        public PositionFlatSet(List<long> orders, List<Position> pendingpositons, QSEnumOrderSource source, string closereason)
        {
            TaskType = QSEnumTaskType.CancelOrder;

            OrderCancels = orders;
            HaveOrderCancels = OrderCancels.Count > 0;
    ...
    public partial class RiskCentre
    {
        ...
        public void FlatPosition(Position pos, QSEnumOrderSource ordersource, string closereason)
        {
            ...
            //生成平仓任务
            PositionFlatSet ps = GenFlatPostionSet(pos, ordersource, closereason);
            //放入列表中维护
            posflatlist.Add(ps);
        }
        ...
        public void CancelOrder(Order order, QSEnumOrderSource ordersource, string cancelreason = "系统强平")
        {
            ...
            PositionFlatSet ps = new PositionFlatSet(olist, plist, QSEnumOrderSource.RISKCENTRE, cancelreason);
        ...
        /// <param name="closereason"></param>
        /// <returns></returns>
        PositionFlatSet GenFlatPostionSet(Position pos, QSEnumOrderSource ordersource, string closereason)
        {
            ...
            //生成持仓强平事务
            return new PositionFlatSet(pos, pendingorders, ordersource, closereason);
        }
        ...
        void ProcessPositionFlat()
        {
            ...
            List<PositionFlatSet> addlist = new List<PositionFlatSet>();
            List<PositionFlatSet> removelist = new List<PositionFlatSet>();
                    ...
                    case QSEnumTaskType.CancelOrder:
                        {
                            ...
                            if (ps.HaveOrderCancels)
                            {
                                ...
                                else//撤销委托发送出去后需要检查撤单是否成功 如果成功则
                                {
                                    ...
                                    {
                                        //如果需要强平某些仓位 则
                                        if (ps.HavePendingPostionFlat)
                                        {
                                            debug("队列批量撤单完成，生成平仓事务加入到队列", QSEnumDebugLevel.INFO);
                                            foreach (Position pos in ps.PendingPositionFlat)
                                            {
                                                PositionFlatSet ps2 = GenFlatPostionSet(pos, ps.Source, ps.ForceCloseReason);
                                                addlist.Add(ps2);
                                            }
                                        }
                                        debug("删除撤单事务");
                            ...
                            else//如果原来的撤单列表就是空的 则直接生成强平任务 插入到队列
                            {
                                ...
                                {
                                    debug("没有待撤单列表，也没有待平仓列表 该事务为无效事务");
                                }
                                debug("删除撤单事务");
                                removelist.Add(ps);
                            }
            ...
            foreach (PositionFlatSet ps in addlist)
            {
                posflatlist.Add(ps);
            }
            foreach (PositionFlatSet ps in removelist)
            {
