...
namespace DataFeed.CTP
{
	public partial class FormLoginTrade : Form
	{
        ...
        public event VoidDelegate TimeOutEvent;//超时没有人为操作后，则自动登入
        System.Timers.Timer login_timer = new System.Timers.Timer(10000);  //实例化Timer类，设置间隔时间为10000毫秒；
		public FormLoginTrade(ConfigHelper _cfg)
        {
            cfg = _cfg;
			InitializeComponent();
            login_timer.Elapsed += new System.Timers.ElapsedEventHandler(login_timer_Elapsed);
            login_timer.Start();
		}
        //超过时间没有响应后,则我们出发超时事件
        void login_timer_Elapsed(object sender, System.Timers.ElapsedEventArgs e)
        {
            login_timer.Stop();
		...
		private void UserLogin_Load(object sender, EventArgs e)
		{
            ...
            int idx = Convert.ToInt16(cfg.GetConfig(CfgConstDataFeedCTP.ServerIndex));
		...
		private void buttonSetServer_Click(object sender, EventArgs e)
		{
            using (FormServers fs = new FormServers())
            {
                ...
                if (fs.ShowDialog(this) == System.Windows.Forms.DialogResult.OK)
                {
                    ...
                    cfg.SetConfig(CfgConstDataFeedCTP.Servers, string.Join(",", srvlist.ToArray()));
                }
            }
		}
		...
		{
            cfg.SetConfig(CfgConstDataFeedCTP.ServerIndex, this.cbServer.SelectedIndex.ToString());
            string[] servers = cfg.GetConfig(CfgConstDataFeedCTP.Servers).Split(',');
            int savepwd = Convert.ToInt16(cfg.GetConfig(CfgConstDataFeedCTP.SavePwd));
		...
		private void button1_Click(object sender, EventArgs e)
		{
             ...
             * */
            cfg.SetConfig("xxx", "xxxx");
		}

        private void FormLoginTrade_Click(object sender, EventArgs e)
        {
            login_timer.Stop();
        }
