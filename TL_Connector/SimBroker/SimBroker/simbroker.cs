...
namespace Broker.SIM
{
    ...
    //
    /// </summary>
    public class SIMTrader: IBroker
    {
        ...
        /// <summary>
        /// 将部分ClearCentre的函数暴露给SimBroker用于获得当前全局的交易信息
        /// </summary>
        ...
        void processPaperTrading(Tick tick)
        {
            try
            {
                ...
                for (int i = 0; i < orders.Length; i++)
                {
                    ...
                    bool filled = o.Fill(tick, UseBidAskFills, false);
                    // 如果没有成交,则直接返回
                    if (!filled)
                    {
                        unfilled.Add(o);
        ...
        { 
             try
            {
                ...
                for (int i = 0; i < orders.Length; i++)
                {
                    ...
                    Tick tick = findTick(o.oSymbol.TickSymbol);
