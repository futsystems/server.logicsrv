...
namespace Broker.Live
{
    ...
    /// 
    /// </summary>
    public class TLBrokerCTPReverse : TLBroker
    {
        ...
        public override void InitBroker()
        {
            ...
            _splittracker.GotFatherOrderEvent += new OrderDelegate(r_NotifyOrder);
            //_splittracker.GotFatherCancelEvent += 
            //委托分拆器更新成交 本地对外通知成交更新
            _splittracker.GotFatherFillEvent += new FillDelegate(r_NotifyTrade);
        ...
        void r_NotifyOrder(Order o)
        {
            Order fo = new OrderImpl(o);
            fo.Size = fo.Size * -1;
        ...
        void r_NotifyTrade(Trade f)
        {
            Trade ff = new TradeImpl(f);
            ff.Side = !ff.Side;
            ff.xSize = ff.xSize * -1;
            NotifyTrade(ff);
        }
