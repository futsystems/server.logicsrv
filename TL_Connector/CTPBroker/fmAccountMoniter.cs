...
namespace Broker.CTP
{
    public partial class fmAccountMoniter : DockContent
    {
        ...
        void initsymbolgrid()
        {
            ...
            this.dtInstruments.Columns.Add("合约乘数", int.MinValue.GetType());
            ...
            symbolgrid.DataSource = dtInstruments;
            
        
        }
        //更新状态信息
        delegate void statedel(EnumProgessState s, string m);
        public void onNewState(EnumProgessState _state, string _msg)
        {
            if (InvokeRequired)
            {
                Invoke(new statedel(onNewState), new object[] { _state, _msg });
            }
            else
            {
        ...
        //更新对象信息>将委托/仓位等信息填充到moniter中
        delegate void objectandkeydel(ObjectAndKey obj);
        public void onNewObject(ObjectAndKey oak)
        {
            if (InvokeRequired)
            {
                Invoke(new objectandkeydel(onNewObject), new object[] { oak });
            }
            else
            {
                ...
                if (oak.Object.GetType() == typeof(ThostFtdcOrderField))
                {
                    ...
                    BetterListViewItem lvi = this.listViewOrder.Items.Find(oak.Key, false).FirstOrDefault();
                    if (lvi == null)
                    {
                        lvi = new BetterListViewItem(field.InstrumentID);	//1:frondid+sessionid,2:ordersysid
                        lvi.Name = oak.Key;
                        ...
                        lvi.SubItems.Add("");							            //成交时间 响应时间
                        lvi.SubItems.Add(field.ExchangeID + "/" + field.OrderSysID.Trim());    //交易所编号+交易所委托编号形成唯一序列号                      
                        lvi.SubItems.Add(field.OrderLocalID);					//报单编号:有成交时对应
                        lvi.SubItems.Add(field.OrderRef);					//报单编号:有成交时对应
                ...
                else if (oak.Object.GetType() == typeof(ThostFtdcTradeField))
                {
                    ThostFtdcTradeField field = (ThostFtdcTradeField)oak.Object;
                    BetterListViewItem lvi = this.listViewTrades.Items.Find(oak.Key, false).FirstOrDefault();
                    if (lvi == null)//如果不存在该成交,则我们新建
                    {
                        lvi = new BetterListViewItem(field.InstrumentID);
                        ...
                        lvi.SubItems.Add(field.ExchangeID + "/" + field.OrderSysID.Trim());
                        ...
                        lvi.SubItems.Add(field.OrderLocalID);           //ctp本地标识

                        listViewTrades.Items.Add(lvi);

                    }
                    else
                    {
                        //如果不存在该委托则我们不做处理
                        string orderkey = field.ExchangeID + "/" + field.OrderSysID.Trim();
                        BetterListViewItem order = this.listViewOrder.FindItemWithText(orderkey);//.FindItemWithText(oak.Key.Trim(), true, 0);
                        if (order != null)
                        {
                            lvi.SubItems[5].Text = field.Price.ToString();					//成交均价
                ...
                else if (oak.Object.GetType() == typeof(ThostFtdcInvestorPositionField))
                {
                    ...
                    BetterListViewItem lvi = this.listViewPosition.Items.Find(oak.Key, false).FirstOrDefault();
                    int unit = (int)this.dtInstruments.Rows.Find(field.InstrumentID)["合约乘数"];	//数量乘积,用于计算价格
                    if (lvi == null)
                    {
                        lvi = new BetterListViewItem(field.InstrumentID);
                        lvi.Name = oak.Key;
                        ...
                        lvi.SubItems.Add(getCost(field.PositionCost, unit, field.Position));	//持仓成本
                ...
                //账户信息
                else if (oak.Object.GetType() == typeof(ThostFtdcTradingAccountField))
                {
                    ThostFtdcTradingAccountField accinfo = (ThostFtdcTradingAccountField)oak.Object;
                    AccountInfo.genAccountInfoData(accinfo);
                    ctTradeAccount1.GotAccountInfo(AccountInfo.genAccountInfoData(accinfo));
        ...
        string getCost(double cost,int unit,int size)
        { 
            string c = (cost / unit / size).ToString("F2");
            if (c.Equals("非数字"))
            {
                return "无持仓";
            }
        ...
        public void Clear()
        {
            ...
            this.listViewPosition.Items.Clear();
            this.listViewTrades.Items.Clear();
        }

        private void symbolgrid_RowPrePaint(object sender, DataGridViewRowPrePaintEventArgs e)
        {
