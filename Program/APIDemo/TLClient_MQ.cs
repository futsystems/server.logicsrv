...
namespace TradingLib.Common
{
    ...
    public class TLClient_MQ
    {
        ...
        public event LoginResponseDel OnLoginResponse;//账户回报
        ...
        public event DebugDelegate OnDebugEvent;//日志输出
        ...
        {
            if (type != MessageTypes.TICKHEARTBEAT)
            {
            ...
            if (packet.Type != MessageTypes.TICKHEARTBEAT)
            {
                
                debug(">>>>>> Recv Packet:" + packet.ToString(), QSEnumDebugLevel.INFO);
            }
            switch (packet.Type)
            {
                /*
