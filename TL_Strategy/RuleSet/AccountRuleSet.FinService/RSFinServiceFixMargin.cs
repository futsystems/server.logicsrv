...
namespace AccountRuleSet
{
    ...
    /// 配资风控策略
    /// 针对福建配资市场,固定保证金模式
    /// 比如5000元可以开1手股指,风控规则为当每手资金为2000元时,则清仓
    /// </summary>
    public class RSFineServiceFixMargin : IAccountCheck
    {
        ...
        private decimal _stopmarginperlot=0;//用于内部使用的值

        string _rawvalue = "";
        public string Value
        {
            get { return _stopmarginperlot.ToString(); }
            set
            {
            ...
            {
                _rawvalue = value;
                _stopmarginperlot = Convert.ToDecimal(value);
        ...
        public bool CheckAccount(out string msg)
        {
            ...
            if(!_acc.AnyPosition) return true;//帐户目前没有持仓

            Position[] pos =  _acc.Positions;
            
            int totallots=0;
            foreach(Position p in pos)
            {
                totallots += p.UnsignedSize;
            }
            if(totallots ==0) return true;//帐户目前持仓总数为0

            decimal marginperlotnow = _acc.NowEquity / totallots;

            /*
            decimal finammount = _acc.FinAmmountTotal;//获得账户配资额
            ...
            **/
            if (marginperlotnow > _stopmarginperlot) return true;//当前每手保证金 大于 我们设定的 强平 保证金 则返回 

            //当每手保证金低于我们所设定的金额时,触发止损动作
            bool ret = false;
            ret = marginperlotnow <= _stopmarginperlot;//当前权益<=强平线 标识
            ...
            if (flatStart)
            {
                ...
                else
                {
                    ...
                    if ((DateTime.Now - _flattime).TotalSeconds > 5 && _cancelnum < CancelRetry)//15秒后任然没有平仓 则再次平仓(累计尝试平仓3次)
                    {
                        ...
                        msg = "RSFineServiceFixMargin 账户:" + _acc.ID + "平仓未成功 再次平仓(" + _cancelnum.ToString() + ")";
        ...
        public string RuleDescription
        {
            get
            {
                return "配资账户每手保证金低于" + _stopmarginperlot.ToString() + "强平仓位并禁止交易";
            }
        }
        ...
        {
            get { return "强平[配资账户风控(固定保证金)]"; }
        }
        public static string Description
        {
            get { return "账户每手保证金低于设定值强平账户并禁止交易"; }
        }
