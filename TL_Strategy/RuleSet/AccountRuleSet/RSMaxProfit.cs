...
namespace AccountRuleSet
{
    public class RSMaxProfit :RuleBase, IAccountCheck
    {
        ...
        {
            msg = string.Empty;
            decimal profit = this.Account.Profit;//获得该账户的当日盈亏
            if (profit < 0 || flatStart ) return true;//如果该账户亏损 则直接返回

            profit = Math.Abs(profit);
            bool ret = profit > _profit;//判断盈利值是否已经超过了设定的数额
        ...
        public override string RuleDescription
        {
            get
            {
                return "账户盈利 " + Util.GetEnumDescription(this.Compare) + " " + _profit.ToString("N2") + "[" + SymbolSet + "]" +"强平仓位并禁止交易";
            }
        }
        ...
        {
            get { return "盈利额度(扣除手续费)大于设定值时,强平持仓并禁止交易"; }
        }
