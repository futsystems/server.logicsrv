    ...
    public class Updater
    {
        ...
        public  bool Detect(string host, int port)
        {
            try
            {
                ...
                getinfo.Unit = Utils.Unit;
                getinfo.AppName = Utils.AppNameUpdate;
                ...
                if (string.IsNullOrEmpty(e.Status))
                {
                    ...
                    //MessageBox.Show("detected result:" + mDetectResult.ToString());
                }
                else
                {
                    Message = e.Status;
        ...
        private void Updating(string address,int port,string appname,bool autoclose)
        {
            ...
            //MessageBox.Show("unit:" + Utils.Unit + " appnameupdate:" + Utils.AppNameUpdate);
            System.Diagnostics.Process.Start("Update.exe", string.Format("{0} {1} {2} {3}",address,port,appname,autoclose));
        }
       
        private UpdateInfo GetLocalInfo()
        {
            UpdateInfo info = new UpdateInfo();
            string filename = AppDomain.CurrentDomain.BaseDirectory + "\\update_info.xml";
            info.Load(filename);
            return info;
        }
    }
